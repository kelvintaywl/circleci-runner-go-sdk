// Code generated by go-swagger; DO NOT EDIT.

package models

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"context"

	"github.com/go-openapi/errors"
	"github.com/go-openapi/strfmt"
	"github.com/go-openapi/swag"
	"github.com/go-openapi/validate"
)

// ResourceClassInfo resource class info
//
// swagger:model ResourceClassInfo
type ResourceClassInfo struct {
	ResourceClassPayload

	// Unique ID of Runner resource-class
	// Required: true
	// Format: uuid
	ID *strfmt.UUID `json:"id"`
}

// UnmarshalJSON unmarshals this object from a JSON structure
func (m *ResourceClassInfo) UnmarshalJSON(raw []byte) error {
	// AO0
	var aO0 ResourceClassPayload
	if err := swag.ReadJSON(raw, &aO0); err != nil {
		return err
	}
	m.ResourceClassPayload = aO0

	// AO1
	var dataAO1 struct {
		ID *strfmt.UUID `json:"id"`
	}
	if err := swag.ReadJSON(raw, &dataAO1); err != nil {
		return err
	}

	m.ID = dataAO1.ID

	return nil
}

// MarshalJSON marshals this object to a JSON structure
func (m ResourceClassInfo) MarshalJSON() ([]byte, error) {
	_parts := make([][]byte, 0, 2)

	aO0, err := swag.WriteJSON(m.ResourceClassPayload)
	if err != nil {
		return nil, err
	}
	_parts = append(_parts, aO0)
	var dataAO1 struct {
		ID *strfmt.UUID `json:"id"`
	}

	dataAO1.ID = m.ID

	jsonDataAO1, errAO1 := swag.WriteJSON(dataAO1)
	if errAO1 != nil {
		return nil, errAO1
	}
	_parts = append(_parts, jsonDataAO1)
	return swag.ConcatJSON(_parts...), nil
}

// Validate validates this resource class info
func (m *ResourceClassInfo) Validate(formats strfmt.Registry) error {
	var res []error

	// validation for a type composition with ResourceClassPayload
	if err := m.ResourceClassPayload.Validate(formats); err != nil {
		res = append(res, err)
	}

	if err := m.validateID(formats); err != nil {
		res = append(res, err)
	}

	if len(res) > 0 {
		return errors.CompositeValidationError(res...)
	}
	return nil
}

func (m *ResourceClassInfo) validateID(formats strfmt.Registry) error {

	if err := validate.Required("id", "body", m.ID); err != nil {
		return err
	}

	if err := validate.FormatOf("id", "body", "uuid", m.ID.String(), formats); err != nil {
		return err
	}

	return nil
}

// ContextValidate validate this resource class info based on the context it is used
func (m *ResourceClassInfo) ContextValidate(ctx context.Context, formats strfmt.Registry) error {
	var res []error

	// validation for a type composition with ResourceClassPayload
	if err := m.ResourceClassPayload.ContextValidate(ctx, formats); err != nil {
		res = append(res, err)
	}

	if len(res) > 0 {
		return errors.CompositeValidationError(res...)
	}
	return nil
}

// MarshalBinary interface implementation
func (m *ResourceClassInfo) MarshalBinary() ([]byte, error) {
	if m == nil {
		return nil, nil
	}
	return swag.WriteJSON(m)
}

// UnmarshalBinary interface implementation
func (m *ResourceClassInfo) UnmarshalBinary(b []byte) error {
	var res ResourceClassInfo
	if err := swag.ReadJSON(b, &res); err != nil {
		return err
	}
	*m = res
	return nil
}
